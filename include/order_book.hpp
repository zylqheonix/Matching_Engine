...
/// Emitted when an IOC order (market or limit) finishes with size that did not
/// rest.
struct IocCanceled {
  uint64_t order_id;
  Side side;
...
public:
  ...
  /// Market orders use IOC: match now; unfilled size is canceled (see
  /// `IocCanceled`). Returns the engine-assigned order id (1-based).
...
private:
  ...
  /// Match aggressive `taker` against the opposite book. Returns unfilled
  /// quantity. `limit_price_constraint`: for limit orders, max trade price
  /// (buy) / min (sell); `nullopt` means market (no price bound).
  template <Side taker_side>
  uint64_t
  match_against_book(const Order &incoming,
